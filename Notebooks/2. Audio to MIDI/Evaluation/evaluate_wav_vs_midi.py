...
# Load the WAV file and extract pitch information
wav_file = "C:/Users/Enqi/Documents/GitHub/IS424/Notebooks/2. Audio to MIDI/Evaluation/alto.wav"
y, sr = librosa.load(wav_file)
pitch_wav, _ = librosa.piptrack(y=y)

# Load the MIDI file and extract pitch information
midi_file = "C:/Users/Enqi/Documents/GitHub/IS424/Notebooks/2. Audio to MIDI/Evaluation/attempt5.mid"
...
for msg in midi_data.play():
    if msg.type == "note_on":
        ...
        if note_number in note_mapping:
            note_name = note_mapping[note_number]
            pitch_midi.append(note_name)

# Calculate pitch accuracy
matching_notes = 0
total_notes = min(len(pitch_wav), len(pitch_midi))

for i in range(total_notes):
    if pitch_wav[i] == pitch_midi[i]:
        matching_notes += 1

accuracy = (matching_notes / total_notes) * 100
...
# for i in range(total_notes):
#     # Compare MIDI note numbers with pitch values from WAV
#     if pitch_wav[i] == pitch_midi[i]:
#         matching_notes += 1
